...
class PostController extends Controller
{
    ...
    public function store(StorePostRequest $request, AdminPostService $posts): RedirectResponse { $post = $posts->store($request->validated(), $request->user()); return redirect()->route('admin.posts.edit', $post)->with('success', 'Bài viết đã được tạo.'); }
    ...
    public function update(UpdatePostRequest $request, Post $post, AdminPostService $posts): RedirectResponse { $posts->update($post, $request->validated(), $request->user()); return redirect()->route('admin.posts.edit', $post)->with('success', 'Bài viết đã được cập nhật.'); }
