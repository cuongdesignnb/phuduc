...
class AdminProductPresentationService
{
    ...
    public function item(Product $product): array
    {
        return [
            'id' => $product->id,
            'name' => $product->name,
            ...
            'price_display' => $this->presentation->money($product->price)['display'],
            'stock' => (int) $product->stock,
            'stock_label' => (int) $product->stock > 0 ? 'Còn hàng' : 'Hết hàng',
            'status' => $this->presentation->status($product->status),
            'image_url' => $this->mediaUrl->resolve($product->cardImage?->image_path),
            'updated_at_display' => $this->presentation->date($product->updated_at),
            'edit_url' => route('admin.products.edit', $product),
            'delete_url' => route('admin.products.destroy', $product),
            'can_delete' => app(ProductReferenceService::class)->canDelete($product),
        ];
    }
    ...
    public function edit(Product $product): array
    {
        return [
            ...
            'specifications' => collect($product->specifications ?? [])->map(fn ($spec) => ['key' => $spec['key'] ?? '', 'value' => $spec['value'] ?? ''])->values()->all(),
            'images' => $product->images->map(fn ($image) => [
                'id' => $image->id,
                'url' => $this->mediaUrl->resolve($image->image_path),
                'is_360' => (bool) $image->is_360,
                'sort_order' => (int) $image->sort_order,
                'file_name' => basename($image->image_path),
            ])->values()->all(),
            'version' => (string) optional($product->updated_at)->toISOString(),
            'delete_references' => app(ProductReferenceService::class)->references($product),
        ];
    }
}
