...
class AdminProductPresentationService
{
    ...
    {
        return ['id' => $product->id, 'name' => $product->name, 'slug' => $product->slug, 'sku' => $product->sku, 'description' => $product->description, 'meta_title' => $product->meta_title, 'meta_description' => $product->meta_description, 'meta_keywords' => $product->meta_keywords, 'price' => (int) round((float) $product->price), 'stock' => (int) $product->stock, 'status' => $product->status, 'specifications' => collect($product->specifications ?? [])->map(fn ($spec) => ['key' => $spec['key'] ?? '', 'value' => $spec['value'] ?? ''])->values()->all(), 'variants' => $product->variants->map(fn ($variant) => ['id' => $variant->id, 'image_id' => $variant->product_image_id, 'name' => $variant->name, 'sku' => $variant->sku, 'price' => (int) round((float) $variant->price), 'stock' => (int) $variant->stock, 'note' => $variant->note, 'status' => $variant->status])->values()->all(), 'images' => $product->images->map(fn ($image) => ['id' => $image->id, 'url' => $this->mediaUrl->resolve($image->image_path), 'is_360' => (bool) $image->is_360, 'sort_order' => (int) $image->sort_order, 'file_name' => basename($image->image_path)])->values()->all(), 'version' => (string) optional($product->updated_at)->toISOString(), 'delete_references' => $references];
    }
}
