...
<template>
    <form class="storefront-card space-y-4 p-5" @submit.prevent="submit">
        <div class="grid gap-4 md:grid-cols-3">
            <FormField id="review-name" label="Họ tên" :error="form.errors.customer_name" required>
            ...
            <FormField id="review-phone" label="Điện thoại" :error="form.errors.customer_phone">
                <template #default="{ id, describedBy }">
        ...
        <fieldset>
            <legend class="mb-2 text-sm font-semibold text-content-primary">Đánh giá</legend>
            <div class="flex flex-wrap gap-2" role="radiogroup" aria-label="Đánh giá">
        ...
        <FormField id="review-content" label="Nội dung" :error="form.errors.content" required>
            <template #default="{ id, describedBy }">
        ...
        <UiButton type="submit" :disabled="form.processing">Gửi đánh giá</UiButton>
    </form>
</template>
