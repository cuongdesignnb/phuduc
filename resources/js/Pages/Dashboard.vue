    ...
    <AuthenticatedLayout>
        ...
        <div class="space-y-6">
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <AdminStatCard :label="metric('pending_orders').label" :value="metric('pending_orders').display" tone="warning" />
            </div>
                ...
                <AdminDataCard title="Doanh thu 6 tháng gần nhất" class="xl:col-span-2">
                    <div v-if="monthlyRevenue.length" class="grid h-56 grid-cols-6 items-end gap-2 sm:gap-4" aria-label="Biểu đồ doanh thu theo tháng">
                        <div v-for="month in monthlyRevenue" :key="month.month" class="flex min-w-0 flex-col items-center gap-2">
                            <span class="max-w-full truncate text-[10px] text-admin-content-muted sm:text-xs">{{ month.revenue_display }}</span>
                            <div class="flex h-36 w-full items-end rounded-t bg-admin-surface-muted">
                                <div class="w-full rounded-t bg-admin-accent" :style="{ height: `${Math.max(month.percentage, month.revenue ? 4 : 1)}%` }" :title="`${month.label}: ${month.revenue_display}`" />
                            </div>
                            <span class="text-[10px] text-admin-content-muted sm:text-xs">{{ month.label }}</span>
                        </div>
                    </div>
                ...
                <AdminDataCard title="Trạng thái đơn hàng">
                    <div v-if="ordersByStatus.length" class="space-y-3">
                        <div v-for="item in ordersByStatus" :key="item.key" class="flex items-center justify-between gap-3">
                            <span class="min-w-0 truncate text-sm text-admin-content">{{ item.label }}</span>
                    ...
                    <template #default>
                        <AdminTable :columns="[{ key: 'order', label: 'Mã đơn' }, { key: 'customer', label: 'Khách hàng' }, { key: 'total', label: 'Tổng tiền' }, { key: 'status', label: 'Trạng thái' }, { key: 'date', label: 'Ngày' }]">
                            <tr v-for="order in recentOrders" :key="order.id" class="border-b border-white/5">
                                <td class="px-3 py-3"><Link :href="order.admin_url" class="font-medium text-admin-accent hover:text-admin-accent-hover focus:outline-none focus:ring-2 focus:ring-admin-focus">{{ order.order_number }}</Link></td>
                                <td class="max-w-36 truncate px-3 py-3 text-admin-content">{{ order.customer_name }}</td>
                                <td class="whitespace-nowrap px-3 py-3 text-admin-content">{{ order.total.display }}</td>
                                <td class="px-3 py-3"><AdminStatusBadge :status="order.status" /></td>
                                <td class="whitespace-nowrap px-3 py-3 text-admin-content-muted">{{ order.created_at_display }}</td>
                            </tr>
                        </AdminTable>
                        <AdminEmptyState v-if="!recentOrders.length" message="Chưa có đơn hàng nào." />
                        <Link :href="route('admin.orders.index')" class="mt-4 inline-flex text-sm text-admin-accent hover:text-admin-accent-hover focus:outline-none focus:ring-2 focus:ring-admin-focus">Xem tất cả đơn hàng</Link>
                    </template>
                </AdminDataCard>
                    ...
                    <div v-if="recentReviews.length" class="divide-y divide-white/5">
                        <article v-for="review in recentReviews" :key="review.id" class="flex items-start justify-between gap-4 py-3 first:pt-0 last:pb-0">
                            <div class="min-w-0">
                                ...
                                <p class="truncate text-sm text-admin-content-muted">{{ review.product_name || 'Sản phẩm đã ẩn' }}</p>
                                <p class="mt-1 text-xs text-admin-content-muted">{{ review.created_at_display }} · {{ review.rating }}/5</p>
                            </div>
                            <AdminStatusBadge :status="review.status" />
                        </article>
                    </div>
                    <AdminEmptyState v-else message="Chưa có đánh giá nào." />
                    <Link :href="route('admin.reviews.index')" class="mt-4 inline-flex text-sm text-admin-accent hover:text-admin-accent-hover focus:outline-none focus:ring-2 focus:ring-admin-focus">Xem tất cả đánh giá</Link>
                </AdminDataCard>
            </div>
            ...
            <AdminDataCard title="Sản phẩm nổi bật">
                <div v-if="topProducts.length" class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
                    <Link v-for="product in topProducts" :key="product.id" :href="product.admin_url" class="min-w-0 border border-admin-border p-3 hover:border-admin-accent/50 focus:outline-none focus:ring-2 focus:ring-admin-focus">
                        <p class="truncate font-medium text-white">{{ product.name }}</p>
                        <p class="mt-1 text-sm text-admin-accent">{{ product.price.display }}</p>
                        <p class="mt-2 text-xs text-admin-content-muted">Kho: {{ product.stock }} · {{ product.review_count }} đánh giá</p>
                    </Link>
                </div>
