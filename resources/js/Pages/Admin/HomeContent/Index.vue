...
const chooseMedia = (media) => { if (pickerTarget.value?.kind === 'config') { setPath(activeSection.value.config, `${pickerTarget.value.path}_media_id`, media.id); setPath(activeSection.value.config, `${pickerTarget.value.path}_is_legacy`, false); } if (pickerTarget.value?.kind === 'item') { pickerTarget.value.item.media_id = media.id; pickerTarget.value.item.image_is_legacy = false; } pickerOpen.value = false; };
const clearConfigMedia = (path) => { setPath(activeSection.value.config, `${path}_media_id`, null); setPath(activeSection.value.config, path, null); setPath(activeSection.value.config, `${path}_is_legacy`, false); };
const clearItemMedia = (item) => { item.media_id = null; item.image = null; item.image_is_legacy = false; };
