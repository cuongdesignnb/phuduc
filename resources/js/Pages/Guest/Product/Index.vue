...
<template>
    ...
    <GuestPageLayout>
        ...
        <section class="relative py-20 overflow-hidden">
            ...
            <div class="absolute top-0 right-0 w-[500px] h-[500px] bg-volt-500/[0.05] rounded-full blur-[120px] pointer-events-none"></div>
        ...
        <section class="py-12">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex justify-end mb-8">
                    <div class="relative w-80">
                        ...
                        <input v-model="search" type="text" placeholder="Tìm kiếm sản phẩm..."
                            class="w-full pl-10 pr-4 py-3 bg-carbon-900/50 border border-white/[0.06] rounded-xl text-sm text-white placeholder-carbon-500 focus:outline-none focus:border-volt-500/40 focus:ring-1 focus:ring-volt-500/20 transition-all duration-300" />
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <Link v-for="(product, i) in products.data" :key="product.id"
                        ...
                        <div class="product-img-wrap">
                            ...
                            <div v-else class="w-full h-full flex items-center justify-center text-carbon-600">
                            ...
                            <div class="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
                                <span class="inline-flex items-center gap-1 bg-volt-500/90 text-carbon-950 px-2.5 py-1 rounded-lg text-[10px] font-display font-bold uppercase tracking-wider">
                                    Chi tiết
                        ...
                        <div class="p-5">
                            <h3 class="text-sm font-semibold text-white group-hover:text-volt-400 transition-colors duration-300 line-clamp-2 leading-snug">{{ product.name }}</h3>
                            <div class="mt-3">
                                <span class="font-display font-bold text-lg" :class="product.price > 0 ? 'text-volt-400' : 'text-industrial-400'">
                ...
                <div v-if="!products.data.length" class="text-center py-24">
                    <svg class="w-16 h-16 mx-auto text-carbon-700 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="0.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                    <p class="text-carbon-500 text-lg">Không tìm thấy sản phẩm nào.</p>
                </div>
                ...
                <div v-if="products.links?.length > 3" class="mt-12 flex justify-center gap-2">
                    <Link v-for="link in products.links" :key="link.label" :href="link.url || '#'"
                        :class="[
                            ...
                            link.active
                                ? 'bg-volt-600 text-carbon-950'
                                : 'bg-carbon-900/50 border border-white/[0.06] text-carbon-400 hover:text-white hover:border-volt-500/30',
